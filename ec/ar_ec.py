...
def compositePano(H2to1, addition, base):
    ...
    #Create a composite image after warping the addition image on top
    #of the image using the homography

    #Note that the homography we compute is from the image to the addition;
    #x_addition = H2to1*x_photo
    #For warping the addition to the image, we need to invert it.

    #Create mask of same size as addition
    mask = np.ones(addition.shape)

    #Warp mask by appropriate homography
    mask_warp = cv2.warpPerspective(mask, np.linalg.inv(H2to1), (base.shape[1],base.shape[0]))
    ...
    #Use mask replace image when intensity is greater
    composite_base = base
    replace_coord = np.all(addition_warp>base,axis=2)
